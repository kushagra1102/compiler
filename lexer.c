...
FILE *getStream(FILE *fp)
{
 ...
 {
    forward_ptr=0;cur_line_no=1;flag_file_end=0;
...
{ // function reads the lexemes and returns all relevant information for the parser
    ...
    while (1)
    {
        ...
        { // start state
            ...
            else if(ch=='\r' || ch=='\t' || ch==' '){ // skip whitespace characters
                inc_func(&forward_ptr,1,&lexeme_ptr,0);
                state=13;
            }
            else if(ch=='\n'){
        ...
        case 1:{
            ...
            else{ // read something else - do not increment 
                state=2;
            }
        }break;
        ...
        case 45:{
            flag_final_state=1;
            copy_string(lexeme,">>");
            assign_token_value(token,ENDEF,lexeme,2,NULL);
        }break;
        case 46:{
            flag_final_state=1;
            copy_string(lexeme,"(");
...
void removeComments(char *testcaseFile, char *cleanFile){
    ...
    cur_line_no=1;forward_ptr=0;
    flag_file_end=0;
    int asteriskCount=0;int isComment=0;//open=1 represents start of the token
    while(1)
    {
        if(current_buffer[forward_ptr]=='\0' || current_buffer[forward_ptr]==(-1))
        {
        ...
        if(asteriskCount==0)
        {
            ...
            {
                fputc(current_buffer[forward_ptr],fptrCleanFile);
            }
            else{
                asteriskCount=1;
            }
